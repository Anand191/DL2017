...
class VariationalAutoencoder(object):
    ...
    def lower_bound(self, x):
        ...
        x_recon = self.mean_x_given_z(z)#self.decoder(z)
        
        kl_loss = -.5 * tf.reduce_sum(1. + z_log_var - tf.square(z_mean) - 
                                      tf.exp(z_log_var), axis=-1)
        xent_loss = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits = x_recon, labels = x),axis=-1)#       
    ...
    def manifold(self,idx,nx=20, ny=20, range_=(-4, 4), ppf=False):
        ...
        canvas = np.vstack([np.hstack([tf.reshape(x,[dim, dim]).eval()
                                       for x in tf.sigmoid(self.decoder(tf.convert_to_tensor(z_row,dtype=tf.float32))).eval()])
                                        for z_row in iter(zs)])
    
        plt.figure(figsize=(nx / 2, ny / 2))
        # `extent` sets axis labels corresponding to latent space coords
        plt.imshow(canvas, cmap="gray", aspect="auto", extent=(range_ * 2))
        if ppf: # no axes
            ax = plt.gca()
            ...
            plt.axis("off")
        plt.tight_layout()
        plt.savefig('Manifold_%s.png' % str(idx))
    
    '''+++++++++++++++++++++++network description begins here+++++++++++++++'''
        ...
        for i in range(int(n_steps)):
            ...
            if (i+1)% 11000==0:
                ...
                for j in range(nn_samples):
                    ...
                    plt.imshow(tf.reshape(samples[j], shape=[28,28]).eval(),cmap='gray')
                    plt.axis('off')
...
#                         z_mu = np.array([[xi,yi]]*model.batch_size).astype('float32')
